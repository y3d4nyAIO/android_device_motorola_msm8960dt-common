...
status_t CameraFlashlight::createFlashlightControl(const String8& cameraId) {
    ...
    if (mCameraModule->getModuleApiVersion() >= CAMERA_MODULE_API_VERSION_2_3) {
        mFlashControl = new ModuleFlashControl(*mCameraModule, *mCallbacks);
        if (mFlashControl == NULL) {
            ALOGV("%s: cannot create flash control for module api v2.3+",
                     __FUNCTION__);
            return NO_MEMORY;
        }
    } else {
...
status_t ModuleFlashControl::hasFlashUnit(const String8& cameraId, bool *hasFlash) {
    ...
    if (mCameraModule->getModuleApiVersion() == CAMERA_MODULE_API_VERSION_2_3)
    {
        ...
        {
            ALOGE("%s: LeEco hack: flash unsupported on front camera", __FUNCTION__);
            *hasFlash = false;
        }
        else
        {
            ALOGE("%s: LeEco hack: flash supported on back camera", __FUNCTION__);
            *hasFlash = true;
        }
    }
    else
